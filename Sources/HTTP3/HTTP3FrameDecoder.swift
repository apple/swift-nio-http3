...
extension HTTP3FrameDecoder: NIOSingleStepByteToMessageDecoder {
    ...
    /// - Note: Any error thrown from here should be treated as a connection-level error.
    @_spi(PackageInternal)
    public mutating func decodeLast(buffer: inout ByteBuffer, seenEOF: Bool) throws -> HTTP3PartialFrameOrUnknown? {
        ...
        @inline(never)
        func leftoverBytesError(location: HTTP3Error.SourceLocation) -> HTTP3Error {
            HTTP3Error(
                code: .leftoverBytes,
        ...
        throw leftoverBytesError(location: .here())
    }
}
