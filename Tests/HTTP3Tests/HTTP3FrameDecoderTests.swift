...
private struct HTTP3FrameByteToMessageDecoder: ByteToMessageDecoder {
    typealias InboundOut = HTTP3PartialFrameOrUnknown

    private var decoder = HTTP3FrameDecoder()

    mutating func decode(context: ChannelHandlerContext, buffer: inout ByteBuffer) throws -> DecodingState {
        guard let frame = try self.decoder.decode(buffer: &buffer) else {
            return .needMoreData
        }
    ...
    mutating func decodeLast(
        ...
        guard let frame = try self.decoder.decodeLast(buffer: &buffer, seenEOF: seenEOF) else {
            return .needMoreData
        }
        context.fireChannelRead(Self.wrapInboundOut(frame))
        return .continue
...
struct HTTP3FrameDecoderTests {
    ...
    @Test
    func decoderPassesVerification() throws {
    ...
    @Test
    func dataFrameIsEmittedInChunks() throws {
    ...
    @Test
    func fullDataFrameInOneBuffer() throws {
        var decoder = HTTP3FrameDecoder()
    ...
    @Test
    func excessivePayloadLengthIsRejected() {
        var decoder = HTTP3FrameDecoder()
    ...
    @Test
    func decodeLastOnCleanBoundaryProducesNothing() throws {
        var decoder = HTTP3FrameDecoder()
    ...
    @Test
    func decodeLastWithNoInputAtAllProducesNothing() throws {
        var decoder = HTTP3FrameDecoder()
        var buffer = ByteBuffer()
        #expect(try decoder.decodeLast(buffer: &buffer, seenEOF: true) == nil)
    }
    ...
    @Test
    func decodeLastWithLeftoverBytesWithoutEOFIsNotAnError() throws {
        var decoder = HTTP3FrameDecoder()
