...
struct HuffmanCodingTests {
    ...
    func assertEqualContents(_ buffer: ByteBuffer, _ array: [UInt8], sourceLocation: SourceLocation = #_sourceLocation)
    {
        #expect(
            buffer.readableBytes == array.count,
        ...
        buffer.withUnsafeReadableBytes { bufPtr in
            array.withUnsafeBytes { arrayPtr in
                #expect(
                    memcmp(bufPtr.baseAddress!, arrayPtr.baseAddress!, arrayPtr.count) == 0,
    ...
    mutating func verifyHuffmanCoding(
        ...
        #expect(numBytes == bytes.count, "Wrong length encoding '\(string)'", sourceLocation: sourceLocation)

        self.assertEqualContents(self.scratchBuffer, bytes, sourceLocation: sourceLocation)
    ...
    mutating func basicCoding() {
        ...
        self.verifyHuffmanCoding("no-cache", [0xa8, 0xeb, 0x10, 0x64, 0x9c, 0xbf])
        self.verifyHuffmanCoding("custom-key", [0x25, 0xa8, 0x49, 0xe9, 0x5b, 0xa9, 0x7d, 0x7f])
        self.verifyHuffmanCoding("custom-value", [0x25, 0xa8, 0x49, 0xe9, 0x5b, 0xb8, 0xe8, 0xb4, 0xbf])

        // these come from http://httpwg.org/specs/rfc7541.html#response.examples.with.huffman.coding
        self.verifyHuffmanCoding("302", [0x64, 0x02])
        self.verifyHuffmanCoding("private", [0xae, 0xc3, 0x77, 0x1a, 0x4b])
        self.verifyHuffmanCoding(
        ...
        self.verifyHuffmanCoding("307", [0x64, 0x0e, 0xff])
        self.verifyHuffmanCoding(
        ...
        self.verifyHuffmanCoding("gzip", [0x9b, 0xd9, 0xab])
        self.verifyHuffmanCoding(
    ...
    mutating func complexCoding() {
        ...
        guard
            ...
            Issue.record("Unable to import base-64 result data")
            return
        }

        // oh if only Data could conform to ContiguousCollection...
        self.verifyHuffmanCoding(text1, Array(encoded1Data))
